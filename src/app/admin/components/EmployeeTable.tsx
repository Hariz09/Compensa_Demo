...
const EmployeeTable: React.FC<EmployeeTableProps> = ({
    ...
    return (
        <div className="bg-gray-800 rounded-lg mb-8">
            {/* Sticky header and filters section */}
            <div className="top-0 z-10 bg-gray-800 rounded-t-lg shadow-lg">
                <div className="p-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {['All', 'Paid', 'Not Paid'].map(status => (
                                        <button
                                            key={status}
                                            onClick={() => handleStatusClick(status === 'All' ? '' : status)}
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {['All', ...departments].map(department => (
                                        <button
                                            key={department}
                                            onClick={() => handleDepartmentClick(department === 'All' ? '' : department)}
                                            className={`px-4 py-2 rounded-full text-sm font-medium transition-colors
                                                ${currentDepartment === (department === 'All' ? '' : department)
                                                    ? 'bg-blue-500 text-white' 
                                                    : 'bg-gray-600 hover:bg-gray-500 text-gray-200'}`}
                                        >
                                            {department}
                ...
                {/* Sticky table header */}
                <div className="px-6 bg-gray-800 shadow-lg">
                        ...
                        <tbody>
                            {filteredEmployees.length > 0 ? (
                                filteredEmployees.map((employee, index) => {
                                    ...
                                    const rowBgColor = index % 2 === 0 ? "bg-gray-700" : "bg-gray-600";
                                    const statusColor = employee.status === "Paid" ? "text-green-600" : "text-red-600";
                                    return (
                                        <tr key={employee.id} className={`${rowBgColor} border-t border-gray-700`}>
                                            <td className="py-2 pl-2">{employee.name}</td>
                                            <td className="py-2 hidden md:table-cell">{employee.position}</td>
                                            <td className="py-2 hidden sm:table-cell">{employee.department}</td>
                                            <td className="py-2">
                                                ${monthData?.totalSalary
                                                    ? monthData.totalSalary.toLocaleString()
                                                    : "N/A"}
                                            </td>
                                            <td className={`py-2 ${statusColor}`}>{employee.status}</td>
                                            <td className="py-2">
                                                <button className="bg-yellow-600 hover:bg-yellow-700 text-white font-bold py-1 px-2 rounded ml-3 mr-2">
